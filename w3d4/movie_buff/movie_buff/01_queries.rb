...
def harrison_ford
  ...
  Movie
    ...
    # .joins(:castings).where(actors: {name: "Harrison Ford"})
    # .where(:castings, "ord != 1")
    # .where
end
...
def directed_by_one_of(them)
  ...
  Movie
    .select(:id, :title)
    .joins(:director) # = join(actors) on actors.id = movies.director_id
    .where(actors: {name: them}) # .where('actors.name IN (?)', them)
end
